...
signal fire(trajectory_obj)

func _ready() -> void:
	mortar_barrel = $mesh/body
	mortar_muzzle = $mesh/body/mesh_muzzle
...
func show_text():
	...
	if last_track != null:
		...
		"Shell distance-battery: %.2f\n" % (last_track.global_position - global_position).length()
	else:
		$ui/shell_status.text = "Shell Status\n" +\
		"No last shell"
